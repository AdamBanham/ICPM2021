...
PROCEDURES_PATIENT_UNIVERSE_SQL = "mimiciii/in/procedures_patient_universe.SQL"
PROCEDURES_CONTROFLOW_SQL = "mimiciii/in/procedures_controlflow.SQL"

PROCEDURES_LOG_CONTROLFLOW_OUT_DIR = "mimiciii/out/procedures/"
PROCEDURES_LOG_PATIENTS_OUT = "patient_universe.csv"
PROCEDURES_LOG_CONTROLFLOW_OUT_CSV = "controlflow_events.csv"
...
if __name__ == "__main__": 
    ...
    subset_patients_sql = update_query_dbname(open(PROCEDURES_PATIENT_UNIVERSE_SQL).read())
    controlflow_patients_sql = update_query_dbname(open(PROCEDURES_CONTROFLOW_SQL).read())
    # get patients
    tqdm.write(f"getting patient universe...")
    patients = run_query(subset_patients_sql,PROCEDURES_LOG_CONTROLFLOW_OUT_DIR+PROCEDURES_LOG_PATIENTS_OUT,res_bucket, res_out)
    ...
    for subset in tqdm(chunked(subject_ids,BATCH_SIZE),desc="collecting controlflow events", total= len(list(chunked(subject_ids,BATCH_SIZE)))):
        temp_df = run_query(
            ...
            PROCEDURES_LOG_CONTROLFLOW_OUT_DIR+"temp.csv",
            res_bucket,
            res_out
        )
        controflow_events_df = pd.concat([controflow_events_df,temp_df])
    #save out controlfow events
    remove_file(PROCEDURES_LOG_CONTROLFLOW_OUT_DIR+"temp.csv")
    tqdm.write(f"saving event universe...")
    controflow_events_df.to_csv(PROCEDURES_LOG_CONTROLFLOW_OUT_DIR+PROCEDURES_LOG_CONTROLFLOW_OUT_CSV,index=False)
